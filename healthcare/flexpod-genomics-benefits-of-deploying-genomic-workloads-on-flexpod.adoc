...
image:flexpod-genomics-image3.png["显示通过重复数据删除过程减少的数据块而元数据保持不变的图表"]
...
image:flexpod-genomics-image4.png["显示 FabricPool 在本地 SSD 和云对象存储之间自动分层活动和非活动数据的图表"]
...
image:flexpod-genomics-image5.png["该图表显示了客户、Cisco、NetApp 和升级团队之间的 FlexPod 协作支持交互"]
