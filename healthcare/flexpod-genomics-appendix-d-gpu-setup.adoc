...
+
image:flexpod-genomics-image18.png["Cisco UCS Manager Inventory GPU 选项卡的屏幕截图，显示计算模式下的 P6 显卡"]

+
image:flexpod-genomics-image19.png["Cisco UCS Manager 的屏幕截图，显示在计算模式下使用 UCSB-GPU-P6-R 的 GPU 库存"]
...
. 此命令在 ESXi 中安装 https://docs.nvidia.com/vgpu/latest/grid-software-quick-start-guide/installing-configuring-vgpu-manager-guest-driver.html#installing-the-nvidia-virtual-gpu-manager-on-vmware-vsphere["NVIDIA vGPU Manager"^]。
+
....
...
+
image:flexpod-genomics-image20.png["vSphere 客户端的屏幕截图，显示配置为 Shared Direct 的 NVIDIA Tesla P6 图形设备"]

+
image:flexpod-genomics-image21.png["选择了 Shared Direct 并选中了重新启动 X.Org 服务器的编辑图形设备设置的屏幕截图"]
...
+
image:flexpod-genomics-image22.png["显示已禁用 EFI 固件和安全启动的 vCenter 虚拟机启动选项的屏幕截图"]
...
+
image:flexpod-genomics-image23.png["显示带有\"编辑配置\"链接的 vCenter 编辑设置高级部分的屏幕截图"]

+
image:flexpod-genomics-image24.png["配置参数对话框的屏幕截图，其中列出了 pciPassthru.64bitMMIOSizeGB 和 pciPassthru.use64bitMMIO 值"]
...
+
image:flexpod-genomics-image25.png["vCenter 编辑设置的屏幕截图，显示为 PCI 设备选择的 NVIDIA GRID vGPU 配置文件 grid_p6-16c"]
...
+
image:flexpod-genomics-image26.png["NVIDIA License Server Licensed Clients 页面的屏幕截图，其中列出了一个虚拟以太网客户端"]
