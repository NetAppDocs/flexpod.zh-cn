...
image:medical-imaging_image3.png["该图显示了配备 UCS、Nexus、AFF 和 MDS 交换机的 FlexPod 医学影像基础设施"]
...
有关详细信息，请参见 https://www.netapp.com/pdf.html?item=/media/8327-ds-3820.pdf["此处"^]。
...
image:medical-imaging_image4.png["显示 SnapCenter 服务器、VMware 集成、复制元数据存储库和应用程序插件的图表"]

NetApp MetroCluster 配置被全球数千家企业用于数据中心内外的高可用性 (HA)、零数据丢失和无中断操作。MetroCluster 是 ONTAP 软件的免费功能，可在不同位置或故障域中的两个 ONTAP 集群之间同步镜像数据和配置。MetroCluster 通过自动处理以下两个目标，为应用程序提供持续可用的存储：通过同步镜像写入集群的数据，实现零恢复点目标 (RPO)。通过镜像配置和自动访问第二个站点的数据，实现接近零的恢复时间目标 (RTO)，MetroCluster 通过位于两个站点的两个独立集群之间的数据和配置自动镜像提供简便性。在一个集群中调配存储时，该存储会自动镜像到第二个站点的第二个集群。NetApp SyncMirror 技术提供所有数据的完整副本，RPO 为零。因此，来自一个站点的工作负载可以随时切换到另一个站点，并在不丢失数据的情况下继续提供数据。有关详细信息，请参见 https://fieldportal.netapp.com/s/article/tr4705-netapp-metrocluster-solution-architecture-and-design-1693982882873["此处"^]。
...
image:medical-imaging_image5.png["显示通过企业服务总线连接到医院系统的社区诊所的图表"]
...
放射科医生通常使用支持图形的高端诊断查看器，这些查看器运行在物理或虚拟桌面基础架构中的高端计算和图形工作站上。如果您即将开始您的虚拟桌面基础架构之旅，请访问了解更多信息 https://www.netapp.com/pdf.html?item=/media/19872-tr-4190.pdf["此处"^]。
...
医学成像系统开始使用 https://www.netapp.com/pdf.html?item=/media/7398-sb-flexpod-datacenter-ai.pdf["基于 GPU 的计算功能"^]通过预处理图像来增强人类工作流程，从而提高效率。典型的企业医学成像系统利用了行业领先的 NetApp 存储效率功能。企业医学成像系统通常使用 RMAN 进行备份、恢复和还原活动。为了提高性能并缩短创建备份所需的时间，Snapshot 技术可用于备份操作，SnapMirror 技术可用于复制。

下图显示了分层架构视图中的逻辑应用程序组件。

image:medical-imaging_image6.png["显示在具有复制灾难恢复的 FlexPod 上的分层医学成像组件的图表"]

下图显示了物理应用程序组件。

image:medical-imaging_image7.png["该图显示了具有 NetApp ONTAP 存储和医学成像服务器的活动和备用 FlexPod 数据中心"]
