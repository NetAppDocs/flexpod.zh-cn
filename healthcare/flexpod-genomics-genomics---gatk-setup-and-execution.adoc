...
image:flexpod-genomics-image8.png["显示从样本采集到测序、变异分析、EHR 集成和预测模型的基因组学工作流程的图表"]
...
+
image:flexpod-genomics-image10.png["显示范围、子网掩码以及管理、VM、NFS 和 iSCSI 用途的 VLAN IP 预留表"]

. 在 ONTAP SVM 上配置基于 iSCSI 的启动 LUN 。
+
image:flexpod-genomics-image9.png["ONTAP System Manager的屏幕截图，显示Healthcare_SVM存储虚拟机中的六个ESXi启动LUN"]

. 将 LUN 映射到 iSCSI 启动程序组。
+
image:flexpod-genomics-image11.png["显示映射到 GenomicsESXi_1 iSCSI 启动程序组的 ESXi_Boot_Lun_1 的屏幕截图"]

+
image:flexpod-genomics-image12.png["ONTAP System Manager 的屏幕截图，显示映射到 GenomicsESXi_2 启动器组的 ESXi 启动 LUN 详细信息"]

. 使用 iSCSI 启动安装 vSphere 7.0 。
. 向 vCenter 注册 ESXi 主机。
+
image:flexpod-genomics-image13.png["vSphere Client 清单的屏幕截图，显示带有 ESXi 主机和虚拟机的 Genomics 文件夹"]

. 在 ONTAP 存储上配置 NFS 数据存储库 `infra_datastore_nfs` 。
+
image:flexpod-genomics-image14.png["ONTAP System Manager 的屏幕截图，显示选定的 Healthcare_Workload_NFS_Vol NFS 卷在线"]

. 将数据存储库添加到 vCenter 。
+
image:flexpod-genomics-image16.png["显示 infra_datastore_nfs NFS 3 数据存储摘要的 vSphere Client 屏幕截图"]

. 使用 vCenter 向 ESXi 主机添加 NFS 数据存储库。
+
image:flexpod-genomics-image15.png["vSphere Client 的屏幕截图，显示已连接且状态正常的 infra_datastore_nfs 主机"]
...
+
image:flexpod-genomics-image17.png["Linux 终端的屏幕截图，显示 /dev/sdb 挂载在 /mnt/genomics 上，使用率为 2%"]
