...
. 单击简介页面上的下一步。
+
image:express-c-series-c190-deploy_image34.png["已选择接受的 vCenter Server Appliance Installer 最终用户许可协议页面的屏幕截图"]

. 选择 Embedded Platform Services Controller 作为部署类型。
+
image:express-c-series-c190-deploy_image35.png["选择了 Embedded Platform Services Controller 的 vCenter Server Appliance Installer 部署类型页面的屏幕截图"]
...
+
image:express-c-series-c190-deploy_image36.png["具有 ESXi 主机凭据的 vCenter Server Appliance Installer Appliance 部署目标窗体的屏幕截图"]

. 输入 vCSA 作为要用于 VCSA 的虚拟机名称和根密码，以设置设备虚拟机。
+
image:express-c-series-c190-deploy_image37.png["vCenter Server Appliance Installer Set up appliance VM 页面的屏幕截图，其中包含 VM 名称和根密码字段"]

. 选择最适合您环境的部署规模。单击下一步。
+
image:express-c-series-c190-deploy_image38.png["已选择 Tiny 的 vCenter Server Appliance Installer Select deployment size 页面的屏幕截图"]
...
+
image:express-c-series-c190-deploy_image39.png["已选择 MGMT-Network 的 vCenter Server Appliance 安装程序配置网络设置页面的屏幕截图"]

. 开始部署设备之前，请查看第 1 阶段的设置。
+
image:express-c-series-c190-deploy_image40.png["vCenter Server Appliance 安装程序准备完成阶段 1 设置审查页面的屏幕截图"]
...
. 在第 2 阶段简介页面上，单击下一步。
+
image:express-c-series-c190-deploy_image41.png["带有\"下一步\"按钮的 vCenter Server Appliance Installer 第 2 阶段简介页面的屏幕截图"]

. 输入 ` \<<var_ntp_id>>` 作为 NTP 服务器地址。您可以输入多个 NTP IP 地址。
. 如果您计划使用 vCenter Server 高可用性（ HA ），请确保已启用 SSH 访问。
. 配置 SSO 域名，密码和站点名称。单击下一步。
+
image:express-c-series-c190-deploy_image42.png["已选择新 SSO 域的 vCenter Server Appliance Installer 第 2 阶段 SSO 配置的屏幕截图"]
...
+
image:express-c-series-c190-deploy_image43.png["选择了 CEIP 选项的 vCenter Server Appliance Installer 配置 CEIP 页面的屏幕截图"]

. 查看设置摘要。单击完成或使用返回按钮编辑设置。
. 此时将显示一条消息，指出在安装开始后，您将无法暂停或停止安装完成。单击确定继续。
+
image:express-c-series-c190-deploy_image44.png["vCenter Server Appliance Installer 警告无法暂停或停止安装的屏幕截图，带有\"取消\"和\"确定\"按钮"]
