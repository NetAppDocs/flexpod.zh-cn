...
+
image:express-c-series-aff220-deploy_image21.png["Cisco CIMC Server BIOS 页面的屏幕截图，顶部菜单中带有 Launch KVM 链接"]
...
. 导航到 https://support.broadcom.com/group/ecx/downloads["Broadcom 支持门户下载网站"^]以获取自定义 ISO。
...
+
image:express-c-series-aff220-deploy_image22.png["ESXi 安装程序选择容量为 15.00 GiB 的 NETAPP LUN C-Mode 存储设备的屏幕截图"]
...
+
image:express-c-series-aff220-deploy_image23.png["为 vSwitch0 选择 vmnic0 和 vmnic1 的 ESXi Network Adapters 菜单的屏幕截图"]
...
+
image:express-c-series-aff220-deploy_image24.png["ESXi Networking Virtual switches选项卡的屏幕截图，显示vSwitch0和iScsiBootvSwitch"]
...
** 单击创建。
+
image:express-c-series-aff220-deploy_image25.png["使用静态 IPv4 设置和 MTU 9000 配置 iScsiBootPG-B 的添加 VMkernel NIC 表单的屏幕截图"]
...
+
image:express-c-series-aff220-deploy_image26.png["选定 iSCSI 软件适配器 vmhba64 的 VMware ESXi 存储适配器页面的屏幕截图"]

. 在动态目标下，单击添加动态目标。
+
image:express-c-series-aff220-deploy_image27.png["在\"动态目标\"下添加动态目标的配置 iSCSI 对话框的屏幕截图"]
...
** 单击保存配置。
+
image:express-c-series-aff220-deploy_image28.png["显示四个 iSCSI 动态目标 IP 地址的屏幕截图，每个 IP 地址使用端口 3260"]
...
. 选择 vSwitch0 。
+
image:express-c-series-aff220-deploy_image29.png["VMware ESXi vSwitch0 页面的屏幕截图，显示 MTU 1500 和两个活动的物理适配器"]
...
. 右键单击端口组选项卡。
+
image:express-c-series-aff220-deploy_image30.png["选中\"端口组\"选项卡并添加可用端口组的 ESXi 网络页面的屏幕截图"]
...
. 单击 VMkernel NIC 选项卡。
+
image:express-c-series-aff220-deploy_image31.png["列出 vmk0 和 vmk1 适配器的 ESXi Networking VMkernel NICs 选项卡的屏幕截图"]
...
** 单击创建。
+
image:express-c-series-aff220-deploy_image32.png["使用静态 IPv4 设置配置 NFS-Network 端口组的添加 VMkernel NIC 表单的屏幕截图"]
...
+
image:express-c-series-aff220-deploy_image33.png["带有静态 IPv4 设置并已选择 vMotion 的\"添加 VMkernel NIC\"对话框的屏幕截图"]
...
+
image:express-c-series-aff220-deploy_image34.png["显示\"新建数据存储区\"按钮的 VMware ESXi 存储页面的屏幕截图"]

. 选择挂载 NFS 数据存储库。
+
image:express-c-series-aff220-deploy_image35.png["新建数据存储区向导的屏幕截图，其中已选择挂载 NFS 数据存储区作为创建类型"]
...
+
image:express-c-series-aff220-deploy_image36.png["显示 NTP 客户端状态和 NTP 服务器的 ESXi Manage System Time  date 页面的屏幕截图"]
...
. 单击保存。
+
image:express-c-series-aff220-deploy_image37.png["已启用 NTP 并输入 NTP 服务器的\"编辑时间配置\"对话框的屏幕截图"]
...
+
image:express-c-series-aff220-deploy_image38.png["显示交换设置的 VMware ESXi Manage System Swap 页面的屏幕截图"]

. 单击编辑设置。从数据存储库选项中选择 infra_swap 。
+
image:express-c-series-aff220-deploy_image39.png["编辑交换配置对话框的屏幕截图，其中 infra_swap 被选为数据存储区，另有保存和取消按钮"]
...
. 重新启动 ESXi 主机。
+
image:express-c-series-aff220-deploy_image40.png["显示成功的 NetApp NFS 插件 VIB 安装和需要重新启动消息的终端"]
