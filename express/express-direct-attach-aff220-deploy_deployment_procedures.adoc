...
按照 https://docs.netapp.com/us-en/ontap-systems/a220/install-setup.html["AFF A220 文档"^]中的控制器物理安装步骤进行操作。
...
+
image:express-direct-attach-aff220-deploy_image7.png["包含IP地址、子网、网关和DNS字段的\"创建IPv4地址块\"对话框的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image8.png["NTP 服务器设置为 10.1.156.4 的\"添加 NTP 服务器\"对话框的屏幕截图"]

. 单击添加 NTP 服务器。
. 输入 ` <switch-b-ntp-ip>` `或 <Nexus B-mgmt-ip>` ，然后单击 OK 。单击确认后的确定。
+
image:express-direct-attach-aff220-deploy_image9.png["列出了两个 NTP 服务器并选择了 America/New_York 时区的 Cisco UCS Manager NTP 设置的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image10.png["屏幕截图，显示配置了上行链路和设备存储角色的 Fabric Interconnect A 以太网端口"]
...
+
image:express-direct-attach-aff220-deploy_image11.png["屏幕截图，显示配置了网络和设备存储角色的 Fabric Interconnect B 以太网端口"]
...
. 单击下一步。
+
image:express-direct-attach-aff220-deploy_image12.png["输入ID为1且名称为vPC-13-Nexus的\"创建端口通道\"对话框的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image13.jpeg["输入了 NFS-VLAN 和 VLAN ID 3170 的\"创建 VLAN\"对话框的屏幕截图"]
...
. 单击确定，然后再次单击确定以创建 VLAN 。
+
image:express-direct-attach-aff220-deploy_image14.png["显示 Cisco UCS Manager 中 VLAN 表以及 VLAN 名称和 ID 的屏幕截图"]
...
. 启用 LLDP 的传输和接收功能。
+
image:express-direct-attach-aff220-deploy_image15.png["屏幕截图，显示启用了CDP和LLDP收发功能的Enable_CDP网络控制策略"]
...
+
image:express-direct-attach-aff220-deploy_image16.png["已启用 CDP 并选择存储 VLAN 的设备接口 1/3 属性的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image17.png["选择iSCSI-B、NFS和本地VLAN的设备接口1/3 VLAN设置的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image18.png["Best Effort MTU 设置为 9216 的 Cisco UCS Manager QoS System Class 设置的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image19.png["选择了 Blade Package 4.0(1b)B 的\"修改包版本\"对话框的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image20.png["创建 MAC 地址块对话框的屏幕截图，MAC 地址为 00:25:B5:32:0A:00，大小为 64"]
...
+
image:express-direct-attach-aff220-deploy_image21.png["\"创建IQN后缀块\"对话框的屏幕截图，后缀为 ucs-host、From 1 和 Size 16"]
...
. 单击确定以创建网络控制策略。单击确定。
+
image:express-direct-attach-aff220-deploy_image22.png["启用了 CDP 和 LLDP 的\"创建网络控制策略\"对话框的屏幕截图"]
...
. 单击确定以创建电源控制策略。单击确定。
+
image:express-direct-attach-aff220-deploy_image23.png["\"创建电源控制策略\"对话框的屏幕截图，已选择 No-Power-Cap 和 No Cap"]
...
. 单击确定创建策略，然后单击确定进行确认。
+
image:express-direct-attach-aff220-deploy_image24.png["Cisco UCS Manager 创建 CPU/内核资格认证对话框的屏幕截图，包含 Xeon 处理器架构和 CPU PID 字段"]
...
. 将一致设备命名更改为已启用。
+
image:express-direct-attach-aff220-deploy_image25.png["启用了 VM-Host 名称和一致设备命名的\"创建 BIOS 策略\"对话框的屏幕截图"]
...
** 处理器 C7 报告：已禁用
+
image:express-direct-attach-aff220-deploy_image26.png["禁用处理器 C 状态和 C1E 的\"创建 BIOS 策略处理器\"选项卡的屏幕截图"]
...
** DRAM 时钟限制：性能
+
image:express-direct-attach-aff220-deploy_image27.png["显示将 Energy Performance 和 DRAM Clock Throttling 设置为 performance 的 BIOS Processor 选项的屏幕截图"]
...
** LV DDR Mode ：性能模式
+
image:express-direct-attach-aff220-deploy_image28.png["将 LV DDR 模式设置为性能模式的\"创建 BIOS 策略 RAS 内存\"页面的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image29.png["选择了User Ack和On Next Boot的Cisco UCS Manager默认维护策略的屏幕截图"]
...
. 单击确定。
+
image:express-direct-attach-aff220-deploy_image30.png["交换矩阵 A 上 vNIC_Template_A 的 Cisco UCS Manager vNIC 模板属性的屏幕截图"]
...
. 单击确定。
+
image:express-direct-attach-aff220-deploy_image31.png["Cisco UCS Manager vNIC 模板 B 设置的屏幕截图，其中选择了 Fabric B、MTU 9000 和 MAC Pool B"]
...
. 单击确定。
+
image:express-direct-attach-aff220-deploy_image32.png["已选择 Fabric A、MTU 9000 和 MAC Pool A56/64 的 Site_01_iSCSI-A vNIC 模板的屏幕截图"]
...
. 单击确定。
+
image:express-direct-attach-aff220-deploy_image33.png["Cisco UCS Manager vNIC 模板 Site_01_iSCSI-B 设置（适用于 MTU 9000 的 Fabric B）的屏幕截图"]
...
. 单击确定将此 vNIC 添加到策略中。
+
image:express-direct-attach-aff220-deploy_image34.png["显示使用 vNIC_Template_A 和 VMWare 适配器策略的 Site-01-vNIC-A 的\"修改 vNIC\"对话框的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image35.png["使用 VLAN 和 MAC 地址设置修改 Site-01-iSCSI-A 的 iSCSI vNIC 对话框的屏幕截图"]
...
. 单击 Save Changes 。
+
image:express-direct-attach-aff220-deploy_image36.png["列出了两个 iSCSI vNIC 的 Site-01-iSCSIBoot LAN 连接策略的屏幕截图"]
...
+
此 VMware ESXi 6.7U1 ISO 可以从 https://support.broadcom.com/group/ecx/downloads["Broadcom 支持门户"^]下载。
...
+
image:express-direct-attach-aff220-deploy_image37.png["使用 HTTP ESXi 安装程序 ISO 远程文件和路径配置的\"创建 vMedia 挂载\"对话框的屏幕截图"]
...
. 单击确定创建策略。
+
image:express-direct-attach-aff220-deploy_image38.png["配置了两个 iSCSI 引导项的 Cisco UCS Manager 引导策略 Site-01-Fabric-A 的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image39.png["创建服务配置文件模板向导的屏幕截图，其中包含名称、更新模板和UUID池字段"]
...
+
image:express-direct-attach-aff220-deploy_image40.png["具有 iSCSI 引导连接且已选择 IQN_Pool 的创建服务配置文件模板网络设置的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image41.png["选择了 Boot-Fabric-A 的 Create Service Profile Template Server Boot Order 页面的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image42.png["列出 Infra-SVM 目标 IQN、别名和 up 状态的 iscsi show 命令输出的屏幕截图"]

. 在 "IPv4 Address" 字段中输入 IP 地址 `iscsi_lif_02a` 。
+
image:express-direct-attach-aff220-deploy_image43.png["输入了 IPv4 地址 192.168.10.62 的\"创建 iSCSI 静态目标\"对话框的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image44.png["具有 IPv4 地址 192.168.10.61 和 LUN ID 0 的\"创建 iSCSI 静态目标\"对话框的屏幕截图"]

. 单击确定以添加 iSCSI 静态目标。
+
image:express-direct-attach-aff220-deploy_image45.png["选择了 iSCSI 静态目标接口的 Set iSCSI Boot Parameters（设置 iSCSI 引导参数）对话框的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image46.png["将 IPv4 地址设置为 192.168.20.62 的\"创建 iSCSI 静态目标\"对话框的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image47.png["使用 IPv4 地址 192.168.20.61 和 LUN ID 0 创建 iSCSI 静态目标对话框的屏幕截图"]

. 单击确定以添加 iSCSI 静态目标。
+
image:express-direct-attach-aff220-deploy_image48.png["显示在设置 iSCSI 引导参数中选择的两个 iSCSI 静态目标接口的屏幕截图"]
...
. 将维护策略更改为默认值。
+
image:express-direct-attach-aff220-deploy_image49.png["使用所选默认策略创建服务配置文件模板维护策略步骤的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image50.png["选择了 Infra-Pool、Down power 状态和默认固件包的创建服务配置文件模板服务器分配的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image51.png["创建服务配置文件模板操作策略页面的屏幕截图，其中选择了 VM-Host BIOS 和 No-Power-Cap"]
...
.. 单击确定以创建服务配置文件。
+
image:express-direct-attach-aff220-deploy_image52.png["\"从模板创建服务配置文件\"对话框的屏幕截图，前缀为 Site-01-Infra-0、数字 2 和根组织"]
...
. 登录到 Broadcom 支持门户网站下载网站： https://support.broadcom.com/group/ecx/downloads["https://support.broadcom.com/group/ecx/downloads"^]。
. 您需要 Broadcom 用户 ID 和密码。查找 VMware vSphere Hypervisor (ESXi) 自定义 ISO。
. 下载 Cisco Custom Image for ESXi 6.7U1 Install CD。
...
. 按 Enter 键。
+
image:express-direct-attach-aff220-deploy_image53.png["选择并连接了 vmnic0 和 vmnic1 的 ESXi 网络适配器屏幕的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image54.png["显示带有 VMkernel 适配器和两个物理适配器的 vSwitch0 详细信息和拓扑的屏幕截图"]

. 选择 VMkernel NIC 选项卡以确认已配置的虚拟适配器。列出的适配器应类似于以下示例：
+
image:express-direct-attach-aff220-deploy_image55.png["列出五个 VMkernel 适配器及其端口组的 ESXi VMkernel NIC 选项卡的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image56.png["选择了iSCSI软件适配器vmhba64的ESXi存储适配器选项卡的屏幕截图"]
...
. 单击保存配置。
+
image:express-direct-attach-aff220-deploy_image57.png["配置 iSCSI 对话框的屏幕截图，显示端口 3260 上的四个动态和静态 iSCSI 目标"]
...
+
image:express-direct-attach-aff220-deploy_image58.png["新建数据存储区向导的屏幕截图，其中已选择挂载 NFS 数据存储区作为创建类型"]
...
.. 单击下一步。
+
image:express-direct-attach-aff220-deploy_image59.png["提供 NFS 挂载详细信息页面的屏幕截图，其中包含 infra_datastore_1 和 NFS 服务器 192.168.104.3"]
...
. 单击完成。此时，数据存储库应显示在数据存储库列表中。
+
image:express-direct-attach-aff220-deploy_image60.jpeg["ESXi 存储数据存储列表的屏幕截图，显示作为 NFS 数据存储挂载的 infra_datastore_2"]
...
+
image:express-direct-attach-aff220-deploy_image61.png["启用 NTP 并输入两个 NTP 服务器地址的\"编辑时间配置\"对话框的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image62.png["VMware ESXi 管理系统交换页面的屏幕截图，显示交换设置和编辑设置"]

. 单击编辑设置。从数据存储库选项中选择 `infra_swap` 。
+
image:express-direct-attach-aff220-deploy_image63.png["选择了 infra_swap 数据存储区以及\"保存\"和\"取消\"按钮的\"编辑交换配置\"对话框的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image64.png["突出显示\"获取 vCenter Server\"按钮的 ESXi 主机导航器的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image65.png["选择了 Embedded Platform Services Controller 的 vCenter Server Appliance Installer 选择部署类型页面的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image66.png["输入了 ESXi 主机凭据的 VCenter Server Appliance Installer Appliance 部署目标页面的屏幕截图"]

. 输入 vCSA 作为 VM 名称以及要用于 VCSA 的根密码，以设置设备 VM 。单击下一步。
+
image:express-direct-attach-aff220-deploy_image67.png["vCenter Server Appliance Installer Set up appliance VM 页面的屏幕截图，其中包含 VM 名称和根密码字段"]

. 选择最适合您环境的部署规模。单击下一步。
+
image:express-direct-attach-aff220-deploy_image68.png["已选择 Tiny 的 vCenter Server Appliance Installer Select deployment size 页面的屏幕截图"]

. 选择 `infra_datastore_1` 数据存储库。单击下一步。
+
image:express-direct-attach-aff220-deploy_image69.png["vCenter Server Appliance Installer Select datastore 页面的屏幕截图，其中在三个数据存储区中列出了 infra_datastore_1"]
...
.. 输入 DNS 服务器。
+
image:express-direct-attach-aff220-deploy_image70.png["带有静态 IPv4 详细信息的 vCenter Server Appliance Installer 配置网络设置页面的屏幕截图"]
...
+
image:express-direct-attach-aff220-deploy_image71.png["显示第 1 阶段 vCenter Server 成功部署的屏幕截图，其中包含\"继续\"按钮"]
...
. 单击确定。
+
image:express-direct-attach-aff220-deploy_image72.png["为 Express 集群启用 DRS 和 vSphere HA 的新集群对话框的屏幕截图"]
...
. 在集群的操作菜单中选择添加主机。
+
image:express-direct-attach-aff220-deploy_image73.png["选择了\"添加主机\"的 Express 集群的 vSphere Client 操作菜单的屏幕截图"]
