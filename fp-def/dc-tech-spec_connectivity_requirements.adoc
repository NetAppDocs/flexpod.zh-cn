...
* 仅建议将独立的 Cisco UCS Fabric Interconnect 设备端口用于 iSCSI 连接。当 NAS 连接与设备端口一起使用时，某些故障情况需要额外的 Nexus / 兼容交换机。
* 不能在核心 FlexPod 组件之间直列放置任何其他设备。


上行链路连接：

* NetApp 存储控制器上的端口必须连接到 Cisco Nexus 9000 系列交换机，以支持虚拟端口通道 (vPC)。
