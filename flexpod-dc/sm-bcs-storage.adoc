...
image:sm-bcs-image22.png["显示 AFF A250 连接到两个站点的 Nexus A 和 Nexus B 交换机的示意图"]
...
image:sm-bcs-image23.png["显示 Nexus 连接的 AFF A250 控制器端口、接口组、VLAN 端口和 LIF 的示意图"]
...
image:sm-bcs-image24.png["显示从 B200 M5 服务器到 AFF A250 控制器的冗余 iSCSI SAN 启动连接的图表"]
...
image:sm-bcs-image25.png["具有两个 IP 地址字段的 ONTAP System Manager Add Intercluster Interfaces 对话框的屏幕截图"]

要将这两个集群建立对等关系，请完成以下步骤：

. 在第一个集群中生成集群对等密码短语。
+
image:sm-bcs-image26.png["具有集群对等权限、有效性和已复制密码短语的\"Generate Passphrase\"对话框的屏幕截图"]

. 在第二个集群中调用对等集群选项，并提供密码短语和集群间 LIF 信息。
+
image:sm-bcs-image27.png["显示\"ONTAP System Manager 对等集群\"对话框的屏幕截图，其中包含密码和集群间 LIF 地址"]

. System Manager 保护 > 概述窗格显示集群对等信息。
+
image:sm-bcs-image28.png["显示集群对等体和调解器状态的 ONTAP System Manager 保护概述的屏幕截图"]
...
image:sm-bcs-image29.png["带有介体连接字段的 ONTAP System Manager 配置介体对话框的屏幕截图"]

提供必要信息后，已配置的 ONTAP 调解器将显示在 System Manager 保护 > 概述窗格中。

image:sm-bcs-image30.png["显示已配置 Mediator 状态的 ONTAP System Manager 保护概述窗格的屏幕截图"]
...
image:sm-bcs-image31.png["\"保护一致性组\"对话框的屏幕截图，其中包含源卷 esxi_a 和目标集群 aff-a250-b"]
...
image:sm-bcs-image32.png["ONTAP System Manager 关系的屏幕截图，显示站点 A 的两个运行正常且同步的一致性组"]

此屏幕截图显示了站点 B 上的一致性组关系

image:sm-bcs-image33.png["站点 B 的 ONTAP System Manager 关系的屏幕截图，显示两个运行正常且同步的一致性组"]

此屏幕截图显示了 CG_infra_datastore_b 组的一致性组关系详细信息。

image:sm-bcs-image34.png["显示 cg_infra_datastore_b 一致性组关系详细信息的 ONTAP System Manager 屏幕截图"]
...
image:sm-bcs-image35.png["Infra-SVM-a的ONTAP卷信息屏幕截图，包括RW、DP和LS卷"]

此屏幕截图显示站点 B 的卷信息

image:sm-bcs-image36.png["Infra-SVM-b 的 ONTAP volume show 输出的屏幕截图，列出了九个卷，包括目标 DP 卷"]
...
image:sm-bcs-image37.png["显示 ONTAP CLI 输出中的站点 A iSCSI igroups 和 LUN 映射的屏幕截图"]

此屏幕截图显示了站点 B 集群的主机 igroup 和 LUN 映射。

image:sm-bcs-image38.png["站点 B igroup 和 LUN 映射命令输出的屏幕截图，适用于 iSCSI VMware 主机"]
