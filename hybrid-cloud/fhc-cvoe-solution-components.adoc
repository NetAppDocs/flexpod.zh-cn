...
FlexPod已获得 EHR 认证，并支持包含Cisco UCS、Intel Xeon 处理器、Red Hat Enterprise Linux (RHEL) 和 VMware ESXiCisco化的平台。该平台，加上 EHR 对运行ONTAP的NetApp存储的高舒适度评级，使您能够通过FlexPod在完全托管的私有云中运行您的医疗保健应用程序，该私有云还可以连接到任何公共云提供商。
...
NetApp Console是一个企业级的、基于 SaaS 的管理平台，它使 IT 专家和云架构师能够使用NetApp云解决方案集中管理其混合多云基础架构。它提供了一个集中式系统，用于查看和管理您的本地和云存储，支持混合云、多云提供商和帐户。有关详细信息，请参阅 link:https://docs.netapp.com/us-en/console-family/index.html["NetApp Console文档"^]。
...
控制台代理实例使控制台能够管理公有云环境中的资源和进程。控制台提供的许多功能都需要控制台代理，它可以部署在云端或本地网络中。
...
link:https://docs.netapp.com/us-en/console-setup-admin/concept-connectors.html["了解有关控制台代理的更多信息"^]。
...
更多详细信息，请参阅link:https://docs.netapp.com/us-en/cloud-manager-cloud-volumes-ontap/["Cloud Volumes ONTAP"^]。
...
有关详细信息，请参阅 link:https://docs.netapp.com/us-en/active-iq-unified-manager/["Active IQ Unified Manager"^]。
...
有关 VMware vSphere 及其组件的更多信息，请参阅link:https://www.vmware.com/products/vsphere.html["VMware vSphere"^]。
...
详细信息请参见link:https://www.vmware.com/products/vcenter.html["VMware vCenter"^]。
...
此混合云解决方案可以扩展到运行 link:http://mysupport.netapp.com/matrix["NetApp 互操作性表工具"^]、link:https://ucshcltool.cloudapps.cisco.com/public/["UCS硬件和软件兼容性"^] 和 link:https://www.vmware.com/resources/compatibility/search.php["VMware 兼容性指南"^] 中定义的受支持版本的软件、固件和硬件的任何 FlexPod 环境。
...
|===
下表显示了控制台和Cloud Volumes ONTAP版本。
