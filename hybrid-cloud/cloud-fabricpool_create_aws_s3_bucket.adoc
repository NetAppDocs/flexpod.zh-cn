...
image:cloud-fabricpool_image10.png["显示有效和无效 FabricPool 聚合到存储桶附加配置的示意图"]
...
. 在云中选择 S3 可扩展存储。
+
image:cloud-fabricpool_image11.png["突出显示云中 S3 可扩展存储的 AWS 控制台搜索结果的屏幕截图"]

. 在 S3 主页上，选择创建存储分段。
. 输入符合 DNS 要求的名称，然后选择要创建存储分段的区域。
+
image:cloud-fabricpool_image12.png["AWS Create bucket 对话框的屏幕截图，其中包含 bucket 名称、区域和 Create 按钮"]
