...
. 转至 https://www.netapp.com/data-infrastructure-insights/["https://www.netapp.com/data-infrastructure-insights/"^]
...
+
image:cloud-insights_image3.png["突出显示了\"新建虚拟机\"的 vSphere 上下文菜单（位于集群操作中）"]
...
+
image:cloud-insights_image4.png["选择了 Linux 和 Red Hat Enterprise Linux 7（64 位）的\"选择来宾操作系统\"向导的屏幕截图"]
...
+
image:cloud-insights_image5.png["VMware 自定义硬件设置的屏幕截图，其中配置了 CPU、内存、磁盘、网络和 CD/DVD 驱动器"]
...
+
image:cloud-insights_image6.png["已选择安装的 Red Hat Enterprise Linux 7.6 启动菜单的屏幕截图"]
...
+
image:cloud-insights_image7.png["显示选定 100 GiB VMware 虚拟磁盘和手动分区选项的屏幕截图"]
...
+
image:cloud-insights_image8.png["使用 25 GiB /opt/netapp 和 /var/log/netapp 卷进行 Red Hat Enterprise Linux 手动分区的屏幕截图"]
...
+
image:cloud-insights_image9.png["启用以太网并输入主机名的 Red Hat Enterprise Linux 网络和主机名屏幕的屏幕截图"]
...
+
image:cloud-insights_image10.png["带有\"重新启动\"按钮的 Red Hat Enterprise Linux 安装完成屏幕的屏幕截图"]

. 系统重新启动后，登录到服务器并将其注册到 Red Hat 订阅管理器。
+
image:cloud-insights_image11.png["显示 Netapp-AU 系统成功注册 Red Hat Subscription Manager 的终端"]

. 附加 Red Hat Enterprise Linux 的可用订阅。
+
image:cloud-insights_image12.png["显示 Red Hat Enterprise Linux 订阅成功附加的终端"]
...
+
image:cloud-insights_image14.png["显示数据收集器和采集单元选项卡的屏幕截图，已选择采集单元"]

. 要创建新的采集单元，请单击右侧的按钮。
+
image:cloud-insights_image15.png["显示\"添加采集单元\"按钮和\"筛选器\"字段的屏幕截图"]
...
+
image:cloud-insights_image16.png["显示 Cloud Insights Linux Acquisition Unit 安装说明和安装程序代码段的屏幕截图"]

. 将此代码片段粘贴到已配置的 Red Hat Enterprise Linux 计算机的 CLI 中，然后单击 Enter 。
+
image:cloud-insights_image17.png["显示在 Linux 上安装 Cloud Insights 采集单元的令牌命令的终端窗口"]

+
安装程序会下载一个压缩的软件包并开始安装。安装完成后，您会收到一条消息，指出采集单元已注册到 NetApp Cloud Insights 中。

+
image:cloud-insights_image18.png["显示成功的 NetApp Cloud Insights 采集单元安装和注册的终端"]
...
+
image:cloud-insights_image19.png["显示 IP、OK 状态和上次报告时间的 NetApp-AU 采集单元摘要的屏幕截图"]

. 要启动向导以添加存储系统，请在摘要页面上单击用于创建数据收集器的按钮。第一页显示可从中收集数据的所有系统。使用搜索栏搜索 ONTAP 。
+
image:cloud-insights_image20.png["在\"添加数据收集器\"页面上显示筛选的 ONTAP 数据收集器选项的屏幕截图"]
...
+
image:cloud-insights_image21.png["配置收集器页面的屏幕截图，其中包含经过验证的 ONTAP 凭据和采集单元设置"]
...
+
image:cloud-insights_image22.png["显示 FlexPod Datacenter 轮询状态的 Cloud Insights 数据收集器行的屏幕截图"]
