...
[role="lead"]
FabricPool 需要 ONTAP 9.2 或更高版本。其他 FabricPool 要求取决于所使用的 ONTAP 版本和要连接的云层。在早于 ONTAP 9.8 的版本中，FabricPool 仅在 SSD 本地层上受支持。在 ONTAP 9.8 中添加了对 HDD 聚合的支持。对于具有固定、相对较小容量的入门级 AFF 平台（如 NetApp AFF C190），FabricPool 对于将非活动数据移动到云层非常有效。
...
* NetApp AFF
* 所有能够运行 ONTAP 9.2 的平台都支持 NetApp FAS FabricPool，但以下平台除外：
+
** FAS8020
...
NOTE: 有关支持 FabricPool 的平台的最新信息，请参见 https://hwu.netapp.com/DetailViews/Home.aspx["NetApp Hardware Universe"^]。

* NetApp ONTAP Select
+

NOTE: NetApp 建议使用全 SSD FabricPool 本地层。
...
** Google Cloud Storage（多区域、区域、近线、冷线、存档）
** IBM Cloud Object Storage （标准，存储，冷存储， Flex ）
** Microsoft Azure Blob Storage （热存储和冷存储）
** NetApp StorageGRID 10.3 及更高版本
+

NOTE: 不支持 Glacier Flexible Retrieval 和 Glacier Deep Archive。
...
** 适用于 AWS 的 NetApp Cloud Volumes ONTAP
** 适用于 Azure 的 NetApp Cloud Volumes ONTAP
...
image:cloud-fabricpool_image6.png["显示两个 NetApp AFF A300 控制器将集群间 LIF 连接到 AWS S3 的示意图"]
...
image:cloud-fabricpool_image7.png["ONTAP CLI 对象存储分析器结果的屏幕截图，显示 PUT 和 GET 延迟以及吞吐量"]
...
FabricPool 许可证（包括现有许可证的额外容量）可以以 1TB 为增量进行购买。
