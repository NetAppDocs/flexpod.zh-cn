...
image:security-ransomware_image2.png["显示采用 Cisco UCS、Nexus、MDS 和 NetApp 存储控制器的高可用 FlexPod 架构的图表"]
...
根据 https://www.ibm.com/think/x-force/threat-intelligence-index-2026-securing-identities-ai-detection-risk-management["2026 IBM X‑Force 威胁情报指数"^]，漏洞利用已成为网络事件的主要原因，人工智能使攻击者能够快速发现和利用安全漏洞。
