...
image:flexpod-fips-image3.png["使用加密模块显示通过网络、WAFL、RAID 和存储的加密写入和解密读取数据流的图表"]
...
image:flexpod-fips-image4.png["显示通过网络、WAFL、RAID 和存储加密模块层进行加密和解密的图表"]
...
image:flexpod-fips-image5.png["显示带有密钥管理器、cryptomod 和 cryptomod_fips 组件的 ONTAP 加密边界的图示"]
